...
public class CalculateUtil {
     ...
     * @return
     */
    public static <T extends Number> BigDecimal safeMultiply(T b1, T ... bn) {
        if (null == b1 || null == bn) {
            return BigDecimal.ZERO;
        }
        BigDecimal result = BigDecimal.valueOf(b1.doubleValue());
        if (null != bn) {
            for (T b : bn) {
                result=BigDecimal.valueOf(b.doubleValue()).multiply(BigDecimal.valueOf(result.doubleValue())).setScale(2, BigDecimal.ROUND_HALF_UP);
            }
        }
        return result;
    }
    ...
    {
        BigDecimal max = arr[0];//定义变量
        for (int x=1; x<arr.length; x++ )
        {
            if (arr[x].compareTo(max)>0)
            {
                max = arr[x];
            }
        }
    ...
    public static BigDecimal getMax(List<BigDecimal> arr)
    {
        BigDecimal max = arr.get(0);//定义变量
        for (int x=1; x<arr.size(); x++ )
        {
            if (arr.get(x).compareTo(max)>0)
            {
                max = arr.get(x);
            }
        }
    ...
    public  static  void  main(String args[]){
        ...
        System.out.println(CalculateUtil.safeMultiply(3,4,2,1));
    }
